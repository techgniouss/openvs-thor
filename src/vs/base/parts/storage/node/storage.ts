	...
	private doConnect(path: string): Promise<IDatabaseConnection> {
		return new Promise((resolve, reject) => {
			import('@vscode/sqlite3').then(sqlite3 => {
				...
				this.logger.error(`[storage ${this.name}] Failed to load @vscode/sqlite3, falling back to JSON storage.`);
				const connection: IDatabaseConnection = {
					db: new JavaScriptDatabase(path, () => {
						resolve(connection);
					}) as any,
					isInMemory: path === SQLiteStorageDatabase.IN_MEMORY_PATH
				};
...
class JavaScriptDatabase {
	...
	all(sql: string, callback: (err: Error | null, rows: any[]) => void): void {
		const rows: any[] = [];
		if (sql.includes('SELECT * FROM ItemTable')) {
	...
	prepare(sql: string): any {
		...
		return {
			...
			run(params: any[]) {
				if (sql.includes('INSERT INTO ItemTable')) {
					for (let i = 0; i < params.length; i += 2) {
						self.data.set(params[i], params[i + 1]);
					}
					self.save();
				} else if (sql.includes('DELETE FROM ItemTable')) {

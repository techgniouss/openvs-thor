...
export class KeyboardLayoutMainService extends Disposable implements INativeKeyboardLayoutService {
	...
	private async _doInitialize(): Promise<void> {
		try {
			...
			// native-keymap is unavailable (e.g. dev builds without compiled native
			// dependencies) - fall back to a null layout so the renderer uses its
			// keyCode-based fallback keyboard mapper instead of failing keybindings.
			console.warn('KeyboardLayoutMainService: native-keymap unavailable, falling back to null keyboard layout.', error);
			this._keyboardLayoutData = { keyboardLayoutInfo: null, keyboardMapping: null };
		}
	}

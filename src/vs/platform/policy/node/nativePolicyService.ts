		...
		try {
			...
			await this.throttler.queue(() => new Promise<void>((c, e) => {
				try {
					...
					this.watcher.value = createWatcher(this.productName, policyDefinitions, (update: any) => {
			...
			this.logService.warn(`NativePolicyService#_updatePolicyDefinitions - Error loading @vscode/policy-watcher:`, err);
		}
	}

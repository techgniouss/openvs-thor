...
export class NativeManagedSettingsService extends Disposable implements INativeManagedSettingsService {
	...
	private async updateWatcher(): Promise<void> {
		...
		try {
			({ createWatcher } = this.watcherFactory ? { createWatcher: this.watcherFactory } : (await import('@vscode/policy-watcher') as { createWatcher: NativePolicyWatcherFactory }));
		} catch (err) {
			// Do not fail configuration initialization when the native module is unavailable
			// (e.g. dev builds without compiled native dependencies) - managed settings are
			// simply not watched in that case.
			this.logService.warn(`NativeManagedSettingsService#updateWatcher - Error loading @vscode/policy-watcher, managed settings are disabled:`, err);
			return;
		}

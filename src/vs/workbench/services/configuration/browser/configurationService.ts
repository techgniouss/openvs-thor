...
export class WorkspaceService extends Disposable implements IWorkbenchConfigurationService, IWorkspaceContextService {
	...
	private async initializeConfiguration(trigger: boolean): Promise<void> {
		...
		const [, application, [local, remote]] = await Promise.all([
			initPolicyConfigurationPromise.catch(error => {
				// A failing policy backend (e.g. missing native module in dev builds) must not
				// block configuration/workspace initialization - policies are then not applied.
				this.logService.error('WorkspaceService: initializing policy configuration failed', error);
				return this.policyConfiguration.configurationModel;
			}),

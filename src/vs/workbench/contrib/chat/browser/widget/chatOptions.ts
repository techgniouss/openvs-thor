...
export class ChatEditorOptions extends Disposable {
	...
	private update() {
		const editorConfig = this.configurationService.getValue<IEditorOptions>('editor') || {} as IEditorOptions;

		// TODO shouldn't the setting keys be more specific?
		const chatEditorConfig = this.configurationService.getValue<IChatConfiguration>('chat')?.editor || {} as any;
		...
		this._config = {
			...
			resultEditor: {
				...
				fontSize: chatEditorConfig.fontSize || 12,
				fontFamily: chatEditorConfig.fontFamily === 'default' || !chatEditorConfig.fontFamily ? (editorConfig.fontFamily || 'monospace') : chatEditorConfig.fontFamily,
				fontWeight: chatEditorConfig.fontWeight || 'normal',
				lineHeight: chatEditorConfig.lineHeight ? chatEditorConfig.lineHeight : ChatEditorOptions.lineHeightEm * (chatEditorConfig.fontSize || 12),
				...
				wordWrap: chatEditorConfig.wordWrap || 'off',

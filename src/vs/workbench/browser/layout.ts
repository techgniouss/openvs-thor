...
class LayoutStateModel extends Disposable {
	...
	load(configuration: ILayoutStateLoadConfiguration): void {
		...
		// OpenVS hosts its chat panel in the auxiliary bar, so the default is
		// wider than upstream's 300px — a chat transcript at 300px wraps code
		// blocks to uselessness. Still capped at a third of the window, and
		// only a default: a user resize persists and wins.
		LayoutStateKeys.AUXILIARYBAR_SIZE.defaultValue = auxiliaryBarForceMaximized ? Math.max(300, mainContainerDimension.width / 2) : Math.min(420, mainContainerDimension.width / 3);

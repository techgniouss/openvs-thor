...
//
// The wire envelope and control-frame vocabulary shared by the relay (`room.ts`) and, in spirit,
// the extension's `src/remote/protocol.ts` (Phase 5) and the PWA's `pwa/app.js`. This file is
// the relay's own copy — the extension cannot import across the package boundary (no bundler,
// no shared `node_modules`), so `pwa/app.js` and `src/remote/protocol.ts` each keep their own
// copy of this shape in sync by hand, which is exactly what `scripts/test-pwa-contract.mjs`
// checks for the PWA half.

/**
 * The envelope every frame is wrapped in, per the plan's "Wire protocol" section:
...
/** Heartbeat request. Answered automatically by the DO's `setWebSocketAutoResponse` without waking a hibernated instance — see `room.ts`. */
 ...
 * still active. Sent back on the same connection the `revoke` frame arrived on (always the
 * host's own socket — see `room.ts`'s `case 'revoke':`).
 */
 ...
 * per `push.ts`'s `shouldPush`, and says so explicitly.
 */
...
/**
 * Sent by the DO to the host when a paired device connects over `/ws/client` — Auth step 6's "new
 * device notice". `firstConnect` distinguishes a device's very first successful connect (worth a
 * desktop notification with a Revoke action) from an ordinary reconnect of an already-seen device
 * (e.g. a paired phone's PWA resuming from background), which should not spam the host every time.
 * Never sent in response to anything the host asked for — there is no host→DO request this
 * answers, it is raised directly by `room.ts`'s `upgradeClient`.
 */
...
/** Type guard for {@link ControlFrame}. Checks only that `c` names a known verb — per-verb field validation is each handler's own job in `room.ts`. */

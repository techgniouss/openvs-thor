...
export function create(deps) {
	 ...
	 * @param {string} className
	 * @param {boolean} attach whether to show it now — a prompt raised in a background tab
	 *   waits in {@link open} until {@link reattach} shows it with its tab.
	 */
	function shell(request, className, attach) {
		...
		if (attach) { deps.container.appendChild(card); }
		return card;
	}

	/** @param {PromptRequest} request */
	function renderApproval(request, attach) {
		const card = shell(request, 'prompt-approval', attach);
		...
		for (const [act, label] of /** @type {['allow'|'always'|'deny', string][]} */ ([
			...
			button.addEventListener('click', () => answer(
				...
				act === 'deny' ? 'Denied' : act === 'always' ? 'Allowed for this run' : 'Allowed',
			));
	...
	/** @param {PromptRequest} request */
	function renderAsk(request, attach) {
		const card = shell(request, 'prompt-ask', attach);
		...
		const submit = () => {
			...
			if (!value) { return; }
			answer(request.id, { answer: value }, `Answered: ${value}`);
		};
	...
	return {
		/**
		 * @param {PromptRequest} request
		 * @param {boolean} [attach] show it now (default) or hold it for its tab — see `shell`
		 */
		render(request, attach = true) {
			if (open.has(request.id)) { return open.get(request.id).card; }
			return request.type === 'askRequest' ? renderAsk(request, attach) : renderApproval(request, attach);
		},
		/**
		 * Re-appends every still-open prompt belonging to `sessionId` (or to no session) — the
		 * transcript view is rebuilt from scratch on a tab switch or fresh transcript, which
		 * used to drop a waiting approval off the screen while its run sat blocked on it.
		 * @param {string} sessionId
		 */
		reattach(sessionId) {
			for (const { request, card } of open.values()) {

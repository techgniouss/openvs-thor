...
**Recommendation embedded in this plan: implement Phase 3 first, ship it, and revisit whether Phase 4 is still wanted before starting it** — not as a refusal, but because Phase 3 already demonstrates the OAuth-proxy pattern end-to-end, and Phase 4's DPAPI/Keychain platform-specific decryption is a genuinely separate, larger body of work (a native module or a hand-rolled DPAPI FFI call in Node, most likely `node:child_process` shelling out to a small compiled helper or an existing crypto-unwrap library, since Node has no built-in `CryptUnprotectData` binding) that this plan has deliberately not designed in full, unlike Phases 1-3 above. If the user re-confirms wanting this after Phase 3, expand this section into full tasks at that point, reading the actual source files rather than working from the README summary this section is based on:
...
### File Structure (placeholder for the follow-up expansion — not implemented by this plan)

- `extensions/openvs-chat/src/providers/webCookie/` — a subdirectory (mirroring the source project's own `web_cookie_providers/` package boundary) rather than a single file, once scoped: platform-specific cookie decryption, the Gemini web adapter, and the `PRODUCTION_ENABLED`-equivalent settings gate belong in separate modules the way they do upstream.
...
- **Taken as a design idea, not yet built (Phase 4):** the `PRODUCTION_ENABLED`-style hard gate, separate from a simple "disabled" flag, for anything this risky.
- **Deliberately scoped smaller than the source project (Phase 3):** no cross-process OAuth-cred-file pool (`~/Documents/<provider>/oauth_creds_N.json`, rotate-on-429) — a single VS Code session has one signed-in identity per provider, not a pool of several harvested accounts, so `KeyRotator`/`getExtraApiKeys` from the companion plan already covers "I have two Copilot accounts" if a user pastes both credentials as primary + extra, without needing a separate pool mechanism.
- **Deliberately not designed in full (Phase 4):** the actual cross-platform cookie decryption and the Gemini-web RPC adapter — flagged above as needing a fresh read of the source files and very likely its own follow-up plan once (and if) the user confirms this specific integration after seeing Phase 3 land.

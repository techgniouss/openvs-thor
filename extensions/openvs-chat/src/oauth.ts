...
export function supportsNativeSignIn(providerId: string): boolean {
	return providerId === 'anthropic' || providerId === 'openai' || providerId === 'openrouter';
}
...
export class OAuthTokenStore {
	...
	async getFreshAccessToken(id: string): Promise<string | undefined> {
		...
		const refreshed = record.type === 'anthropic'
			? await refreshAnthropic(record.refresh)
			: await refreshOpenAI(record.refresh);
...
function toRecord(type: StoredOAuth['type'], json: any): StoredOAuth {
	return {
		type,
...
async function refreshOpenAI(refreshToken: string): Promise<StoredOAuth | 'invalid'> {
	...
	return toRecord('openai', { ...json, refresh_token: json.refresh_token ?? refreshToken });
}
...
export async function signInOpenAI(store: OAuthTokenStore): Promise<OpenAISignInResult | undefined> {
	...
	await store.set('openai', toRecord('openai', json));
	return { mode: 'chatgpt' };
}

...
//
// v4: shell assets moved from cache-first to network-first (see the `fetch` handler). Under v3 a
// redeployed relay served its new index.html (navigations were already network-first) next to
// the *old* cached app.js and styles.css — markup and script from two different releases, which
// breaks the moment an element id changes and otherwise just keeps showing the old UI forever.
const CACHE_NAME = 'openvs-relay-shell-v4';
const SHELL_ASSETS = [
 ...
 *  - Everything else (the shell's own JS/CSS/icons) is network-first, refreshing the cache on
 *    every success and falling back to it offline — the same policy as the navigation above, so
 *    the page and its scripts always come from the same release.
 */
self.addEventListener('fetch', event => {
	...
	event.respondWith(fetch(request).then(response => {
		if (response.ok) {
			...
			event.waitUntil(caches.open(CACHE_NAME).then(cache => cache.put(request, copy)).catch(() => { /* best-effort */ }));
		}
		return response;
